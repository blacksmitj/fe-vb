...
import { useDraftsCount, useReverificationCount } from "@/hooks";
...
export function NavMain() {
  ...
  const { data: reverificationCount } = useReverificationCount();
  ...
  const hasReverificationNotification = reverificationCount !== undefined && reverificationCount > 0;
      ...
      <SidebarGroup>
        ...
        <SidebarMenu>
          ...
          <SidebarMenuItem>
            <SidebarMenuButton
              asChild
              isActive={pathname.startsWith("/re-verification")}
              tooltip="Verifikasi Ulang"
            >
              <Link href="/re-verification" className="flex justify-between items-center w-full">
                <div className="flex items-center gap-2">
                  <div className="relative flex items-center justify-center">
                    <RefreshCw />
                    {isCollapsed && hasReverificationNotification && (
                      <span className="absolute -top-1 -right-1 flex h-2 w-2 rounded-full bg-amber-500 ring-2 ring-sidebar animate-gentle-glow animate-gentle-bounce" />
                    )}
                  </div>
