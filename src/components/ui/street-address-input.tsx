...
export function StreetAddressInput({
  ...
  return (
    <div className="w-full space-y-1.5">
      ...
      >
        ...
        <InputGroupAddon align="inline-end" className="flex items-center gap-1 pr-1.5">
          ...
          <InputGroupButton
            ...
            title={
              isFormatted
                ? "Klik untuk membatalkan (kembali ke input awal)"
                : isInvalid
                ? "Klik Magic Wand untuk memperbaiki format otomatis"
                : "Format Alamat & RT/RW (Magic Wand)"
            }
          >
            {isFormatted ? (
              <>
      ...
      {showHelperText && !isInvalid && (
        <div className="flex items-center justify-between text-[11px] text-muted-foreground px-0.5">
          <span>
            {helperText || (
              <>
                Klik tombol <span className="font-semibold text-foreground">Format Magic</span> untuk penataan otomatis & RT/RW.
              </>
            )}
          </span>
          {isFormatted && (
            <span className="text-amber-600 dark:text-amber-400 font-medium flex items-center gap-1">
              <Check className="size-3" /> Diformat (klik "Batal" jika tidak pas)
            </span>
          )}
        </div>
      )}
